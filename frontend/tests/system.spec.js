...
test.describe('系统管理 - 用户管理', () => {
  ...
  test('【批量操作】下拉按钮存在，点击可展开下拉菜单', async ({ page }) => {
    const batchBtn = page.locator('.el-dropdown').filter({ hasText: '批量操作' }).first();
    await expect(batchBtn).toBeVisible();
    // 触发按钮本身可见
    await expect(batchBtn.locator('button')).toBeVisible();
  });

  // 注意：用户管理页面无导出、刷新按钮，此处跳过
  ...
  test('【删除】按钮可见可点击，弹出确认框', async ({ page }) => {
    await page.waitForSelector('.el-table__row', { timeout: 5000 });
  ...
  test('新增对话框中【确定】按钮存在', async ({ page }) => {
    await page.locator('button').filter({ hasText: '新增' }).first().click();
    await page.waitForSelector('.el-dialog');
    const confirmBtn = page.locator('.el-dialog .el-dialog__footer button').filter({ hasText: '确定' });
    await expect(confirmBtn).toBeVisible();
    await expect(confirmBtn).toBeEnabled();
  });

  // ===== 分页按钮 =====
  test('分页组件可见，总页数显示', async ({ page }) => {
...
test.describe('系统管理 - 角色管理', () => {
  ...
  test('【新增角色】按钮可见可点击，弹出对话框', async ({ page }) => {
    const addBtn = page.locator('button').filter({ hasText: '新增' }).first();
    await expect(addBtn).toBeVisible();
    await addBtn.click();
    await expect(page.locator('.el-dialog')).toBeVisible();
...
test.describe('系统管理 - 菜单管理', () => {
  ...
  test('【新增菜单】按钮可见可点击，弹出对话框', async ({ page }) => {
    const addBtn = page.locator('button').filter({ hasText: '新增' }).first();
    await expect(addBtn).toBeVisible();
    await addBtn.click();
    await expect(page.locator('.el-dialog')).toBeVisible();
...
test.describe('系统管理 - 部门管理', () => {
  ...
  test('【新增部门】按钮可见可点击，弹出对话框', async ({ page }) => {
    const addBtn = page.locator('button').filter({ hasText: '新增' }).first();
    await expect(addBtn).toBeVisible();
    await addBtn.click();
    await expect(page.locator('.el-dialog')).toBeVisible();
...
test.describe('系统管理 - 岗位管理', () => {
  ...
  test('【新增岗位】按钮可见可点击，弹出对话框', async ({ page }) => {
    const addBtn = page.locator('button').filter({ hasText: '新增' }).first();
    await expect(addBtn).toBeVisible();
    await addBtn.click();
    await expect(page.locator('.el-dialog')).toBeVisible();
...
test.describe('日志管理 - 登录日志', () => {
  ...
  test('页面加载完整', async ({ page }) => {
    await expect(page.locator('.el-pagination, .el-table').first()).toBeVisible();
  });
});
...
test.describe('系统设置 - 参数配置', () => {
  ...
  test('【新增参数】按钮可见可点击，弹出对话框', async ({ page }) => {
    const addBtn = page.locator('button').filter({ hasText: '新增' }).first();
...
test.describe('数据字典', () => {
  ...
  test('字典数据页面可正常访问', async ({ page }) => {
    ...
    await page.waitForLoadState('networkidle');
    await page.waitForTimeout(1500);
    await expect(page.locator('.el-table, .el-pagination').first()).toBeVisible();
  });
});
